        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if(response.success===true && response.result.graphNodes !==null && response.result.graphEdges !==null) {
                    ...
                    function makedd(fromx,fromy,tox,toy,dd){
                        new twaver.Animate({
                            ...
                            onUpdate: function (value) {
                                dd.setLocation(value.x, value.y);
                            },
                            onDone:function(){
                                box.remove(dd);
                            }
                    ...
                    for(var i=0;i<graphEdges.length;i++){
                        for(var j=0;j<graphNodes.length;j++){
                            if(parseInt(graphNodes[j]['viz_size'])==100){
                                if(graphEdges[i]['target']==graphNodes[j].id){
                                    var fromposx=getPos(graphNodes[j]['viz_position_x'])+16;
                                    var fromposy=getPos(graphNodes[j]['viz_position_y'])+16;
                                    for(var k=0;k<graphNodes.length;k++){
                                        if(parseInt(graphNodes[k]['viz_size'])==52){
                                            if(graphEdges[i]['source']==graphNodes[k].id){
                                                var toposx=getPos(graphNodes[k]['viz_position_x'])+12;
                                                var toposy=getPos(graphNodes[k]['viz_position_y'])+12;
                                                makedd(fromposx,fromposy,toposx,toposy,ddArr[i]);
                                            }
                                        }else if(parseInt(graphNodes[k]['viz_size'])==4){
                                            if(graphEdges[i]['source']==graphNodes[k].id){
                                                var toposx=getPos(graphNodes[k]['viz_position_x'])+9;
                                                var toposy=getPos(graphNodes[k]['viz_position_y'])+9;
                                                makedd(fromposx,fromposy,toposx,toposy,ddArr[i]);
                                            }
                                        }
                                    }
                                }
                            }else if(parseInt(graphNodes[j]['viz_size'])==52){
                                if(graphEdges[i]['target']==graphNodes[j].id){
                                    var fromposx=getPos(graphNodes[j]['viz_position_x'])+12;
                                    var fromposy=getPos(graphNodes[j]['viz_position_y'])+12;
                                    for(var k=0;k<graphNodes.length;k++){
                                        if(graphEdges[i]['source']==graphNodes[k].id){
                                            var toposx=getPos(graphNodes[k]['viz_position_x'])+9;
                                            var toposy=getPos(graphNodes[k]['viz_position_y'])+9;
