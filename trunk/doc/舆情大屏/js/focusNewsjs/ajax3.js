...
var vb = new Vue({
    ...
    data:{
        ...
        profileImageURL:'img/focusNews-img/head.png',
        //simpleReport:'',
        newExp:22536477,
        newForward:6477,
        newOneF:4077,
...
function getshuju() {
    $.ajax({
        type: "GET",
        //async: false,
        url: "http://192.168.3.44:8080/front/weiboAnalysisResult",
                ...
                if(vb.simpleReport){
                    ...
                    function choose() {
                        ...
                        //太多了
                            ...
                            vb.newThreeF = reArr[4][0];
                            //用户情绪指数
                            vb.userEmotion = reArr[reArr.length-1][0];
                        }
                    }
//"消息曝光量<span title="曝光量表示所有转发用户的总粉丝数">1547379</span>，共计转发<b>4799</b>次，
// 其中一转<b>1562</b>次，二转<b>1505</b>次，三转<b>352</b>次，四转<b>138</b>次，四层以上总共<b>77</b>次。
// 其他、广东地区参与转发人数较多。用户情绪指数为<b>35</b>，较为正面。疑似水军比例为<b>38.19%</b>，有较多垃圾账号参与转发。"
                    choose ();
                ...
                //关键传播账号所有信息数组
                vb.keyArr = response.result.data.top100User.splice(0,10);
            ...
            if(response.success===true && response.result.graphNodes !==null && response.result.graphEdges !==null) {
                ...
                function readdnode(){
                    if(view){
                        document.getElementById('dianlutu').removeChild(view);
                    }
                    //创建twaver容器
                    ...
                    //var pos=[
                    //    {x:setpos(600,20),y:setpos(350,20)},
                    //    {x:setpos(600,20),y:setpos(350,20)},
                    //    {x:setpos(600,20),y:setpos(350,20)},
                    //    {x:setpos(600,20),y:setpos(350,20)},
                    //    {x:setpos(600,20),y:setpos(350,20)},
                    ...
                    var xsrannode=[];
                    for(var i=0;i<20;i++){
                        xsrannode.push(setrandom(xsnode.length,0));
                    }
                    //xsnode.length=20;
