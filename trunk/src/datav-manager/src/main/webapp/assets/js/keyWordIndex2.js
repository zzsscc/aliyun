        ...
        //删除专题
        var removeLi=null;
        var delztPopup=$('#delZtModal');
        var removeZt=$('.remove-zt');
        removeZt.click(function(){
            for(var i=0;i<zt.find('li').length;i++){
                if($(zt.find('li')[i]).hasClass('active')){
                    removeLi=$(zt.find('li')[i]).find('a');
                }
            }
        })
        //删除专题确定
        ...
        addPopup.find('.sure-button').on('click',function () {
            ...
            if(itemName && itemName.search(/\n/ig)!= -1){
                ...
                for(var i = 0;i<itemList.length;i++)
                {
                    ...
                    if(strlen(itemTemp) < 10)
                    {
                        alert("每个关键字长度至少为5个中文字符或10个英文字符");
                        return;
                    }

                    var reg = new RegExp("^[0-9]*$");
                    if(reg.test(itemTemp))
                    {
                        alert("每个关键字不能为纯数字!");
                        return;
                    }
                ...
                itemName = '["'+itemName+'"]';
            }else if(itemName && itemName.search(/\n/ig)== -1){
                if(strlen(itemName) < 10)
                {
                    alert("关键字长度至少为5个中文字符或10个英文字符");
                    return;
                }
