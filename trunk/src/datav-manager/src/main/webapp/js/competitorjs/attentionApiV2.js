...
//                加入后来的对象
            ...
            function bindBom(){
                for(var j=0;j<chulires.length;j++){
                    ...
                    (function(j){
                        setTimeout(function(){
                            ajaxPSEvent(time,chulires[j].topicId,1,chulires[j].name);
                        },intervalTime);
                    })(j);
                }
            }
            ...
            for(var i=0;i<childArr.length;i++){
                ...
                switch (childArr[i].position) {
                    case 0 :
                        childArr[i].priority = 1;
                        break;
                    case 4 :
                        childArr[i].priority = 2;
                        break;
                    case 6 :
                        childArr[i].priority = 3;
                        break;
                    case 1:
                        childArr[i].priority = 4;
                        break;
                    case 2:
                        childArr[i].priority = 5;
                        break;
                    case 3:
                        childArr[i].priority = 6;
                        break;
                    case 5:
                        childArr[i].priority = 7;
                        break;
                }
            ...
            //copy一个childArr数组，对copyArr动刀
            var copyArr = [];
            for(var i=0;i<childArr.length;i++){
                copyArr.push(childArr[i]);
            }

            //去掉最大优先级对象后，余留的数组
            var remainArr = copyArr;
            //再来是竞品的指针图
            function lunhui(paramArr,fn,option,map){
                //最大优先级
                var maxPri = 100;
                var maxTopicId;
                var maxName;
                ...
                for(var i=0;i<paramArr.length;i++){
                    //如果存在优先级，则用优先级进行比较
                    if(paramArr[i].priority){
                        if(paramArr[i].priority < maxPri){
                            maxPri = paramArr[i].priority;
                            maxTopicId = paramArr[i].topicId;
                            maxName = paramArr[i].name;
                            maxIndex = i;
                        }
                    }
                }
                //如果最大优先级还是等于100的话就说明只剩下专题
                if(maxPri === 100){
                ...
                //否则就是有竞品的
                    ...
                    fn(time,maxTopicId,1,option,map,maxName);
                    paramArr.splice(maxIndex,1);
                    remainArr = paramArr;
                }
            }

            //第一次优先级轮回
            lunhui(remainArr,ajaxPSemotion,eA2Option,eA2);
            //第二次优先级轮回
            lunhui(remainArr,ajaxPSemotion,eA3Option,eA3);
            //第三次优先级轮回
            lunhui(remainArr,ajaxPSemotion,eA4Option,eA4);
