    ...
    $.ajax({
        ...
        success:function(data){
            ...
            for(var i=0;i<superArr.length;i++){
                if(superArr[i].position === -1 ){
                    $('.ps-topic').css({
                        background:'url('+superArr[i].imgUrl+')',
                        backgroundSize:'100% 100%'
                    });
                    $('#d-img-url').val(superArr[i].imgUrl);
                    $('.topic-dialog .d-m-img').attr('src',superArr[i].imgUrl);
                    $('.island2 .i-title').text(superArr[i].name).attr('data-id',superArr[i].topicId);
                }
            }
            ajaxPSget();
        },
...
$('.product-dialog .ok').on('click',function(){
    if($('.drap-name').text() !== '选择关注品牌'){
        ...
        //提交修改内容
        //obj.priority = $('.img-ct .product').eq(PSIndex).attr('data-priority-me');
